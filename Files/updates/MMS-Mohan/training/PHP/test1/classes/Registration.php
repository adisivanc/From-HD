  ...
  class Contacts
  {
	...
	var $Institute;
	 ...
	 function insertContact()
	 {
		...
		$query="INSERT INTO `".TBL_REGISTRATION."` (`Name`,`MobileNo`,`EmailAddress`,`Institute`,`AddedDate`) VALUES ('$this->Name','$this->MobileNo','$this->EmailAddress','$this->Institute','$sqlDateTime')";			
	...
	function getContactDtl()
	{
		...
		if(count($sub_qry)>0) $subqry= " where ".implode(' and ',$sub_qry);
		 $query="select ".$this->fields." from ".TBL_REGISTRATION." ".$subqry.$sort_qry;	
		if($this->Id=='')
	...
	{
		$query = "select EmailAddress from ".TBL_REGISTRATION." where EmailAddress='".$this->EmailAddress."'";	
		if($this->Id!='')
		{
		$query= "select * from ".TBL_REGISTRATION." where Id  NOT IN ('".$this->Id."') AND EmailAddress ='".$this->EmailAddress."'";
		}
