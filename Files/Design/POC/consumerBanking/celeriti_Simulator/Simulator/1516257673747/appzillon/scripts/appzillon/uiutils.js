Apz.prototype.accordionAction = function(obj) {
	...
	if (proceed != false) {
		if($(obj).parent().find('.height-transition:first').hasClass("height-transition-hidden")){
    ...
    if (proceed != false) {
        ...
        if ($(obj).parent().hasClass('acco')) {
            if ($(obj).parent().hasClass('acco') && !$(obj).next('div').hasClass('active')) {
                ...
                $(liObj).siblings('li').removeClass('current');
        ...
        //for desktop
            ...
            if (!(jqueryObj).hasClass("current")) {
                ...
                jqueryObj.siblings('li').removeClass("current");
            }
            ...
            if (parentDiv.hasClass('nrsp')) {
                ...
                activeTabCont = tabobj.eq(jqueryObj.index());
                activeTabCont.addClass('active').siblings("ul").addClass('current');
            }
        }
    }
     ...
     if (currentRec<=0) {
	  ...
	  $('#'+container + '_info').text('Showing ' +currentRec+ ' to ' +lastRec+ ' of ' +totalRecs+' entries');	
	   ...
	   tableObj.find("tr.child").each(function(){
		...
		this.initDataTables($('#scr__' +proc.appId+'__'+ proc.scr + '__main .responsive:visible'));
		...
		if(jqueryObj.length > 0) {
			jqueryObj.each(function() {
				...
				if(!$(this).parents(".crb-tabl:first")[0].__resizeListeners__) {
					apzObj.addResizeListener($(this).parents(".crb-tabl:first")[0], apzObj.resizeTableHandler);
				}
			});
		}
	    ...
	    tableDiv.attr("oldWidth",tableDiv.width());
	    var apzObj = this;
	    if (jqueryObj.length > 0) {
		            ...
		            if (proceed) {
		                ...
		                jqueryObj.find('tbody').off("click.apzevent").on('click.apzevent', 'td.dticon', function() {
		                    ...
		                    if (iconObj.length > 0) {
		                        ...
		                        $(this).html('').append('<svg class="icon ' + oldTargetClass + ' px24"><use xlink:href="#' + oldTargetClass + '"></use></svg>');
		                    }
	...
	Apz.prototype.initDraggables = function(proc) {
		...
		if(jQuery.fn.accordion){
			jQuery("#scr__"+proc.appId+'__'+ proc.scr + "__main .topnav").accordion({
	            accordion: false,
	            speed: 500,
	            closedSign: '<svg class="icon icon-downarrow px12"><use xlink:href="#icon-downarrow"></use></svg>',
	            openedSign: '<svg class="icon icon-uparrow px12"><use xlink:href="#icon-uparrow"></use></svg>'
	        });
		}
    ...
    this.initModalEvent(proc);
	    ...
	    if (tblObj.is(":visible")) {
	        ...
	        if (isResponsive) {
                /*var left = tableWidth;
	            var right = tableCntWidth;
	            if (tableCntWidth > tableWidth) {
	                left = tableCntWidth;
	                right = tableWidth;
	            }*/
	            //if (!tblObj[0].hasAttribute("tableaction") && left % right > 15 && tableOldWidth != tableCntWidth) {
	            //if (left % right > 15 && tableOldWidth != tableCntWidth) {
		...
		if(params.dateType == "JQUERY") {
			this.initJqueryDates(params.id);
		...
		//}
	}, Apz.prototype.initJqueryDates = function(id) {
		$("#"+id).datepicker({
		...
		var sortType = false;
		var icon = "icon-sort-desc";
		var svgElm = $(btnElm).children('svg');
		if (svgElm.hasClass("icon-sort-desc")) {
			sortType = true;
		}
		svgElm.parents('th:first').siblings('th').find('button svg').remove();
		var appender = '<svg class="ett-icon icon-sort px14"><use xlink:href="#icon-sort"></use></svg>';
		svgElm.parents('th:first').siblings('th').find('button').append(appender);
		var params = {};
		...
		appender = '<svg class="ett-icon '+icon+' px14"><use xlink:href="#'+icon+'"></use></svg>';
		svgElm.remove();
		$(btnElm).append(appender);
    ...
    var iconObj = drdnDivObj.children('span');
	   ...
	   */
		...
		if ($(obj).attr("disabled") != undefined) {
            ...
            $el.removeClass("height-transition-hidden sno").addClass('htou');
