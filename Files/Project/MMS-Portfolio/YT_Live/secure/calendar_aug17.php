...
if($_POST['act']=="updateCalendarEvent")
{
	...
	'grades','visibility_type','teachers_id','added_by','event_time','from_time_24','to_time_24','send_notification','notification_to','from_time','to_time','event_color','id');
...
if($_POST['act']=="saveCalendarEvent")
{
	...
	if($_POST['multiple_day']=="YES")
	{
		...
		foreach($aryDates as $date)
		{
				...
				$postArrs = array('event_date','event_name','address','event_desc','school_id',
				'grades','visibility_type','teachers_id','added_by','event_time','from_time_24','to_time_24','send_notification','notification_to','from_time','to_time','event_color');
				$stringArr = array('event_name','event_desc');
				foreach($postArrs as $K=>$V) 
				{
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" style="margin:10px auto;background-color:#999" class="popuptbl" >
  <tr>
  ...
  <tr style="background-color:#FFF">
    ...
    <td>
    	<input type="radio" name="visibility_type" id="visibility_type" value="I" <? if($update!=0 && $cV->visibility_type=="I"){?> checked="checked"<? }?> onclick="getTeachers(this.value,'<?=$event_id?>','')"/>Internal
			<input type="radio" name="visibility_type" id="visibility_type" value="P" <? if($update!=0 && $cV->visibility_type=="P"){?> checked="checked"<? }?> />Parents
			<input type="radio" name="visibility_type" id="visibility_type" value="PU" <? if($update!=0 && $cV->visibility_type=="PU"){?> checked="checked"<? }elseif($update==0){  }?> />Public
			<input type="radio" name="visibility_type" id="visibility_type" <? if($update!=0 && $cV->visibility_type=="S"){?> checked="checked"<? }?> value="S" />School
    </td>
  </tr>  
    ...
    <div class="fullsize">
        ...
        <div class="fullsize newsletter_outer">
          ...
          <div class="newsletter_right border_theme bgwhite" style="width:78.8%; ">
                ...
                <tr><th><span id="school_name"></span> Calendar</th></tr>
