...
if($_REQUEST['blog_post_id']!='' || ($blogId!="" && $blogId!="undefined")){
	...
	$_SESSION['BlogPreview']['publish_date'] = date("M d, Y", strtotime($rs_BlogDtl->publish_date));
        ...
        <td width="70%" style="padding:10px;">
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="studentinnertbl">
                <tr>
                    ...
                    <input type="checkbox" name="category_id[]" id="category_id[]" value="<?=$N->id?>" <?=(in_array($N->id, $category_idArr) || in_array($N->id, $Category))?"checked":""?> /> <?=$N->category_name?>
                    <? }
                ...
                <tr>
                    <td id="publishdaterr">Publish Date</td>
                    <td><input type="text" class="txtbox datepicker"  name="publish_date" id="publish_date" value="<?=($_SESSION['BlogPreview']['publish_date']=="" || $_SESSION['BlogPreview']['publish_date']=="0000-00-00" || $_SESSION['BlogPreview']['publish_date']=="1970-01-01")?"":stripslashes($_SESSION['BlogPreview']['publish_date'])?>"/></td>
                </tr>
                
                <tr>
                    <td id="featureerr">Featured Article?</td>
...
function submitBlogPost(id, value) {  
	...
	if($('#publish_date').val()=='' || $('#publish_date').val()==' '){ err=1; $('#publishdaterr').addClass('txterror'); } else { $('#publishdaterr').removeClass('txterror'); }
