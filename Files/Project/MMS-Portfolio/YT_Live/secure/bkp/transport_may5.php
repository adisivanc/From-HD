...
if($_POST['act']=="saveTransDtls") {
	...
	$Id = $_POST['id'];
	if($_POST['transType']=="B") {
		if($_POST['id']!="") Transportation::updateBusRoute($schoolId, check_input(ucwords($_POST['route_name'])), check_input($_POST['description']), $Id); 
		else Transportation::insertBusRoute($schoolId, check_input(ucwords($_POST['route_name'])), check_input($_POST['description']));
	}
	if($_POST['transType']=="V") {
		if($_POST['id']!="") $updateVehicle = Transportation::updateVehicle($schoolId, check_input($_POST['route_id']), check_input($_POST['vehicle_reg']), check_input($_POST['reg_number']), $_POST['id']);
		else $rsVehicle = Transportation::insertVehicle($schoolId, check_input($_POST['route_id']), check_input($_POST['vehicle_reg']), check_input($_POST['reg_number']));
	}
	exit();
}
...
if($_POST['act']=="deleteTransDtls") {
	...
	if($_POST['transType']=="B") { 
		...
		foreach($vehicleRoute as $K=>$V) Transportation::updateVehicleByfield("route_id","",$V->id);
	}
	
	if($_POST['transType']=="V") { 
		Transportation::deleteVehicle($_POST['id']);
		$driverVehicle = Transportation::getDriverByVehicleId($_POST['id']);
		foreach($driverVehicle as $K=>$V) Transportation::updateDriverByfield("vehicle_id"," ",$V->id);
	}
...
if($_POST['act']=="driverFrmSubmit") {
	...
	if($error==0) {
		...
		if($driver_id!="" && $driver_id!="undefined") {
			Transportation::updateDriver($school_id, $vehicleIds[0], ucwords($_POST['driver_name']), check_input($_POST['driver_address']), $_POST['driver_mobile'], $_POST['driver_blood_group'], $_POST['driver_license_number'], $license_expiry_date, $vehicleIds[1], $driver_id);
			$rs_driver_id = $driver_id;
		} else {
			$rs_driver_id = Transportation::insertDriver($school_id, $vehicleIds[0], ucwords($_POST['driver_name']), check_input($_POST['driver_address']), $_POST['driver_mobile'], $_POST['driver_blood_group'], $_POST['driver_license_number'], $license_expiry_date, $vehicleIds[1]);
		}
...
function saveTransFrm(type, id) { //alert(type); alert(id);
	...
	if(type=="B") {
		if($('#route_name').val()==''){ err=1; $('#route_name').addClass('boxerror'); } else { var route_name = escape($.trim($('#route_name').removeClass('boxerror').val())); }
		if($('#description').val()==''){ err=1; $('#description').addClass('boxerror'); } else { var description = escape($.trim($('#description').removeClass('boxerror').val())); }
		param = param+'&route_name='+route_name+'&description='+description;
	}
	
	if(type=="V") {
		if($('#vehicle_route_id').val()==''){ err=1; $('#vehicle_route_id').addClass('boxerror'); } else { var route_id = escape($.trim($('#vehicle_route_id').removeClass('boxerror').val())); }
...
function saveDriverFrm(id, e) {
	...
	if($('#driver_vehicle_id').val()==undefined){ err=1; $('#driver_vehicle_id_tab').addClass('txterror'); } else { $('#driver_vehicle_id_tab').removeClass('txterror'); }
...
function saveStudentRoute(school_id, route_id) {
	...
	if(routeAction=="" || routeAction==undefined) { err=1; alert('Please choose action'); }
