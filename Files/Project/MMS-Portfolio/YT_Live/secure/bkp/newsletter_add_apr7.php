...
<div class="fullsize_pad padtb10 lineht1_8">
    ...
    <div class="pull_right marginright20">Newsletter Date
    	<input type="text" class="txtbox datepicker" id="ns_date" name="ns_date" value="<?=($rs_circular->ns_date!="" && $rs_circular->ns_date!="0000-00-00")?date("m/d/Y", strtotime($rs_circular->ns_date)):""?>" />
    </div>
</div>
<div class="newsletter_cntr">
    ...
    <div class="fullsize bgtheme"><? $rs_ApplyTo = unserialize($rs_circular->apply_to); ?>
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="moduletbl">
          ...
          <tr>
            ...
            <td style="line-height:25px; padding:0px;" valign="top">
                ...
                <table border="0" cellpadding="0" cellspacing="0" width="100%">
                    <tr>
                        <td>
                        <h4><b><input type="checkbox" name="ApplyTo[T]" id="" class="uncheck_list_T" value="T" onclick="uncheckList('T')" <? if(in_array('T',$title)) { echo 'checked'; } ?>/> Teachers</b></h4>
                        <div style="margin-left:10px;" id="teacher_schl_list">
                            <?	
                                $sch_obj = new School();
								$rs_school = $sch_obj->getSchoolDtls();
                                /*if($_SESSION['SchoolId']=="A") {
                                    $rs_school = $sch_obj->getSchoolDtls();
                                } else {
                                    $sch_obj->id= $_SESSION['SchoolId'];
                                    $rs_school[] = $sch_obj->getSchoolDtls();
                                }*/
                                ...
                                if(count($rs_schools)>0) {
                                    ...
                                    ?>
                                        <? //if($_SESSION['SchoolId']=="A") { ?>
                                        <input type="checkbox" name="ApplyTo[T][TS:<?=$V->id?>]" id="" class="uncheck_sub_list_T" value="TS:<?=$V->id?>" <? if(in_array("TS:".$V->id,$schlArr)) { echo 'checked'; } ?>/> <b><?=$V->school_name?></b>
                                        <? //} ?>
                                        <?
                                        $grd_obj = new Grade();
                                        ...
                                        if(count($rs_grades)>0) {
                                            ...
                                            ?>
                                                <input type="checkbox" name="ApplyTo[T][TS:<?=$V->id?>][]" id="" class="uncheck_sub_list_T" value="TSG:<?=$vv->id?>" <? if(in_array("TSG:".$vv->id,$gradeArr)){ echo 'checked'; } ?>/> <?=$vv->grade_name?><br />
                                            <?
                                            }
                                        }
                                        ?>
                        ...
                        <h4><b><input type="checkbox" name="ApplyTo[S]" id="" class="uncheck_list_S" value="S" onclick="uncheckList('S')" <? if(in_array('S',$title)){ echo 'checked'; } ?>/> Students</b></h4>
                        <div style="margin-left:10px;" id="teacher_schl_list">
                            <?	
                                if(count($rs_schools)>0) {
                                    foreach($rs_schools as $K=>$V) {
                                    ?>
                                        <? //if($_SESSION['SchoolId']=="A") { ?>
                                        <input type="checkbox" name="ApplyTo[S][SS:<?=$V->id?>]" id="" class="uncheck_sub_list_S" value="SS:<?=$V->id?>" <? if(in_array("SS:".$V->id,$schlArr)){ echo 'checked'; } ?>/> <b><?=$V->school_name?></b>
                                        <? //} ?>
                                        <?
                                        $grd_obj = new Grade();
                                        $grd_obj->school_id = $V->id;
                                        ...
                                        if(count($rs_grades)>0) {
                                            foreach($rs_grades as $kk=>$vv) {
                                            ?>
                                                <input type="checkbox" name="ApplyTo[S][SS:<?=$V->id?>][]" id="" class="uncheck_sub_list_S" value="SSG:<?=$vv->id?>" <? if(in_array("SSG:".$vv->id,$gradeArr)){ echo 'checked'; } ?>/> <?=$vv->grade_name?><br />
                                            <?
                                            }
                                        }
                                        ?>
                    ...
                    <tr>
                        ...
                        <h4><b><input type="checkbox" name="ApplyTo[NS]" id="subscriber_list" class="uncheck_sub_list_NS" value="NS" <? if(in_array('NS',$title)){ echo 'checked'; } ?>/> Subscribers</b></h4>
                        </td>
                    </tr>
          ...
          <tr id="senddatetab" style="display:none;">
    ...
    <div class="fullsize txtwhite txtcenter f18">
        ...
        <? if($rs_circular->status=="N") { ?><div class="bgbrown pull_right marginleft20 margintb10 cursor padlr20 padtb10" onclick="submitNSForm('S');"><strong>SENT</strong></div> <? } ?>
        <? if($rs_circular->status=="D" || $rs_circular->status=="") { ?><div class="bgbrown pull_right marginleft20 margintb10 cursor padlr20 padtb10" onclick="submitNSForm('D');"><strong>SAVE</strong></div> <? } ?>
        <div class="bgbrown pull_right margintb10 cursor padlr20 padtb10" onclick="submitNSForm('N');"><strong>ADD</strong></div>
        <? } ?>
    </div> 
...
function uncheckList(type, action) {
	var list = $('input[class=uncheck_list_'+type+']:checked').val();
	
	if((list!="" && list!=undefined) || (action=="New")){
		$('.uncheck_sub_list_'+type).attr("checked", true);
	} else {
		$('.uncheck_sub_list_'+type).attr("checked", false);
	}
	
	if(action=="New") {
		$('.uncheck_list_'+type).attr("checked", true);
		$('.uncheck_list_'+type).attr("checked", true);
		$('.uncheck_list_'+type).attr("checked", true);
	}
}

function uncheckMailList(type) { 
	var list = $('input[class=circular_group_'+type+']:checked').val();
	
	if(list!="" && list!=undefined){
		$('.circular_sub_group_'+type).attr("checked", true);
	} else {
		$('.circular_sub_group_'+type).attr("checked", false);
	}
}
...
function submitNSForm(action) {
	...
	$('#ns_submit_action_type').val(action);
	...
	if($('input[name="send_date_position"]:checked').val()=="Y") if($('#send_date').val()==''){ err=1; $('#send_date').addClass('boxerror'); } else { $('#send_date').removeClass('boxerror'); }
