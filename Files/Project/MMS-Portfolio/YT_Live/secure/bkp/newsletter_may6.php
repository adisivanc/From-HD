...
if($_POST['act']=="saveCircular") {
	...
	if($send_date_position=="Y") {
		if($_POST['send_date']!="" && $_POST['send_date']!="0000-00-00") $senddate=date("Y-m-d", strtotime(trim($_POST['send_date']))); else $senddate="";
		$send_date = $senddate;
		$send_time = $_POST['send_time_hh'].":".$_POST['send_time_mm']." ".$_POST['send_time_ampm'];
		$send_time_format = date("H:i:s", strtotime($send_time));
	}
	$status = trim($_POST['ns_submit_action_type']);
	if($status=="U") $status = trim($_POST['newsletter_db_status']);
	...
	$ns_date = $nsdate;
	$circular_send_to = ($_POST['send_to_option']);
	$is_configure_url = trim(($_POST['is_configure_url']));
	if($is_configure_url=="Y") { 
		$configure_url = trim(addslashes($_POST['configure_url']));
		$button_type = trim(addslashes($_POST['button_type']));
	}
	
	if($_POST['newsletter_db_id']!="" && $_POST['newsletter_db_id']!="undefined") {
		$rs_id = $_POST['newsletter_db_id'];
		Circulars::updateCircular($rs_id, $ns_type, $title, $subject, $header_type, $welcome_note, $welcome_text, $no_of_inner_image, $welcome_description, $conclusion_text, $regards_text, $regards_from_text, $send_date, $send_time, $send_time_format, $send_date_position, $status, $apply_to, $is_highlight_text, $term_calender_position, $ns_date, $circular_send_to, $is_configure_url, $button_type, $configure_url);
	} else {
		$rs_id = Circulars::insertCircular($ns_type, $title, $subject, $header_type, $welcome_note, $welcome_text, $no_of_inner_image, $welcome_description, $conclusion_text, $regards_text, $regards_from_text, $send_date, $send_time, $send_time_format, $send_date_position, $status, $apply_to, $is_highlight_text, $term_calender_position, $ns_date, $circular_send_to, $is_configure_url, $button_type, $configure_url);
	}
	...
	if($button_type=="I") {
		if($_FILES['button_image']['size'] > 0) {
			$FileArr = $_FILES['button_image'];	
			$rExt1 = array('jpg','jpeg','png','gif','pdf','xls','doc','docx','xlsx','ppt','pptx');
			$FileObj = new FileUpload();
			$FileResult = $FileObj->AssignAndCheck(array('FileRef'=>$FileArr, 'Extension'=>implode(',', $rExt1),'PathPrefix'=>NEWSLETTER_BUTTON_PATH));
			...
			elseif($FileResult['Type']==2) {
				$btn_file_upload = true;
			}
		}
		if($btn_file_upload) {
			$button_file_name = "button_".$rs_id;
			$FileObj->AssignFileName($button_file_name);
			$filepath_button = $FileObj->Upload();
			Circulars::updateCircularByField('button_details', $filepath_button, $rs_id);
		}
	}
	if($button_type=="T") {
		$button_text = trim(addslashes($_POST['button_text']));
		Circulars::updateCircularByField('button_details', $button_text, $rs_id);
	}
...
if($_POST['act']=='showCircularEmail'){
    ...
    <table width="100%" border="0" style="background:#FFF;" class="popuptbl" cellpadding="0" cellspacing="0">
        ...
        <tr>
			...
			<? if($rs_circular->circular_send_to=="A") echo "This Newsletter will send to all the students"; 
			if($rs_circular->circular_send_to=="N") echo "This Newsletter will send to new students only";
			if($rs_circular->circular_send_to=="O") echo "This Newsletter will send to old students only"; ?>
            </td>
        </tr>
        ...
        <td id="circular_td">
        	<table>
                ...
                <tr>
                    <td colspan="2">
						...
						if(is_array($rs_ApplyTo)) {
							...
							?>
                            	<div style="border-bottom:1px dotted #CCCCCC; line-height:30px; width:50%; float:left;">
                                <input type="checkbox" name="circular_group[]" id="" class="mail_parent_<?=trim($k1)?>" value="<?=trim($k1)?>" onclick="uncheckMailList('<?=$k1?>', '', 'P')" checked/> <?=$titlename?> 
                            <?
								if(is_array($v1)) {
									...
									?>
                                        ...
                                        <input type="checkbox" name="circular_group[]" id="mail_sub_parent_<?=$k1?>_<?=trim($slname[1])?>" class="mail_parent_child_<?=$k1?>" value="<?=trim($k2)?>" onclick="uncheckMailList('<?=$k1?>', '<?=trim($slname[1])?>', 'C')" checked/> <?=$rs_schl->school_name?> 
                                	<?
										if(is_array($v2)) {
											...
											if($k1=="S") {
												$isNewStudent=""; 
												if($rs_circular->circular_send_to=="N") $isNewStudent = "Y"; else if($rs_circular->circular_send_to=="O") $isNewStudent = "N";
												$student_obj = new Student(); 
												if($isNewStudent!="") $student_obj->is_new_student=$isNewStudent;
												$student_obj->grade_id=$gdname[1];
												$rs_student = $student_obj->getAllStudentDtls();
												$resultCount = "(".count($rs_student).")";
											}
											if($k1=="T") {
												$rs_teacher = Teacher::getTeachersByGrade($gdname[1]);
												$resultCount = "(".count($rs_teacher).")";
											}
											$rs_log = new Circulars();
											$rs_log->grade_id = $gdname[1];
											$rs_log->circular_id = $rs_circular->id;
											$rs_mail_log = $rs_log->getCircularEmailLogDtl(); 
											if(count($rs_mail_log)>0) $isGradeSent = ""; else $isGradeSent = "checked";
											?>
                                            	<div style="margin-left:10px;"><input type="checkbox" name="circular_group[]" id="" class="mail_child_<?=$k1?>_<?=trim($slname[1])?> mail_child_<?=$k1?>" value="<?=trim($v3)?>:<?=$rs_schl->id?>" <?=$isGradeSent?>/> <?=$rs_grade->grade_name?> <?=$resultCount?></div>
                                            <?
											}
										}
									?> 
                            ...
                            <?
							if($k1=="S") echo '</div><div style="border-bottom:1px dotted #CCCCCC; line-height:30px;">';
							}
						}
						?>
                ...
                <tr>
                    <td colspan="2"> This newsletter has been sent already. Would you like to resend, those who did not receive this newsletter?
                    	<input type="radio" name="resend_not_receive" id="resend_not_receive" value="Y" /> Yes
                    	<input type="radio" name="resend_not_receive" id="resend_not_receive" value="N" checked/> No
                    </td>
                </tr>
			...
			foreach($emailArr as $K_Mem=>$V_Mem){
					...
					$sentArr[]=$emailAddress;	
						...
						if($arrayCnt>=500 || $sentCnt==$totalCnt){
							...
							$rs_NewsDtl = Circulars::insertCircularLog($_POST['nid'], $members, "", $currentdate);
        ...
        <tr>
        	<td id="test_circular_td">
                <table border="0" width="100%" cellpadding="0" cellspacing="0">
                    ...
                    <tr>
                      <td  width="40%" height="30" colspan="2" align="center">
                      <div class="combutton pull_right padlr10 padtb10 txtbold letterspac f18" onclick="updateStatus('<?=$_POST['circularId']?>', 'S')">Save</div>
                   </tr>
                </table>
             </td>
         </tr>   
...
<div class="fullsize">
    ...
    <div class="fullsize menu_head padtb10">
        ...
        <div class="pull_right f24 padtop50">
            <a href="newsletter.php"><div class="combutton pull_right f24 cursor">Add Newsletter</div></a>
        </div>
    </div>
            ...
            <div class="newsletter_left"> <!-- Newsletter Circular -->
            	<div class="newsletter_submenu txtwhite">
					<div class="circular_outer">
                    	<div class="newcircular_head tabbtn" id="tabbtn_N" onclick="showMenuList('N');">New Circulars<span></span></div>
                        <ul class="listoption txttheme" id="curcularmenutab_N" style="padding-right:10px; float:left;"></ul>
                    </div>
                    
                    <div class="circular_outer" style="clear:both;">
                    	<div class="newcircular_head tabbtn" id="tabbtn_D" onclick="showMenuList('D');">Draft Circulars <span></span></div>
                        <ul class="listoption txttheme" id="curcularmenutab_D" style="padding-right:10px; float:left;"></ul>
                    </div>
                    
                    <div class="circular_outer" style="clear:both;">
                    	<div class="newcircular_head tabbtn" id="tabbtn_S" onclick="showMenuList('S');">Sent Circulars <span></span></div>
                        <ul class="listoption txttheme" id="curcularmenutab_S" style="padding-right:10px; padding-bottom:20px;"></ul>
                    </div>
                </div>
...
function bulkCircularForm() {
   ...
   err=0;
	...
	var resend_not_receive = $('input[name=resend_not_receive]:checked').val();
	
	if(err==0) {
		$('#circular_td').html('<span style="font-size:24px; padding:20px; color:#BD220A">sending please wait...</span>');
		ajax({
			a:'newsletter',							
			b:'act=sendCircularEmail&nid='+nid+'&from_address='+from_address+'&Subject='+Subject+'&circular_group='+circular_group+'&addschedule='+addschedule+'&send_unsub='+send_unsub+'&resend_not_receive='+resend_not_receive,
			c:function(){},
			d:function(data) {
				//alert(data);
...
function uncheckMailList(type, action_id, action) { 
	if(action=="P") {
		var list = $('input[class=mail_parent_'+type+']:checked').val(); 
		if((list!="" && list!=undefined) || (action=="New")) { $('.mail_parent_child_'+type).attr("checked", true); $('.mail_child_'+type).attr("checked", true); } 
		else { $('.mail_parent_child_'+type).attr("checked", false); $('.mail_child_'+type).attr("checked", false); }
	} 
	else {
		var sub_list = $('input[id=mail_sub_parent_'+type+'_'+action_id+']:checked').val(); 
		if((sub_list!="" && sub_list!=undefined) || (action=="New")) { $('.mail_parent_'+type).attr("checked", true); $('.mail_child_'+type+'_'+action_id).attr("checked", true); } 
		else { $('.mail_child_'+type+'_'+action_id).attr("checked", false); }
	}
}
