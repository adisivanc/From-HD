...
<table width="100%" border="0" cellspacing="1" cellpadding="0" class="gradetbl" bgcolor="#dab791" style="clear:both;">
    <tr bgcolor="#FFFFFF">
        ...
        <? } ?>
        <th width="21%" align="left" scope="col">Father Details</th>
        <th width="21%" align="left" scope="col">Mother Details</th>
...
if(count($rs_studentArr)>0){
    ...
    ?>
        <tr bgcolor="#FFFFFF">
            ...
            <div class="btn_group">
            <img src="images/mail_icon.png" alt="" title="Mail" class="cursor" align="absmiddle" onclick="studentActions(<?=$V->id?>, 'M')" />
            <img src="images/view_icon.png" alt="Quick View Student" title="Quick View Student" class="actionicons" onclick="studentActions(<?=$V->id?>, 'QV')" />
            <? if($actionPage!="Grade") { ?><img src="images/detail_view.png" alt="Detailed View Student" title="Detailed View Student" class="actionicons" onclick="studentActions(<?=$V->id?>, 'DV')" /><? } ?>
            <img src="images/edit_icon1.png" alt="Quick Edit Student" title="Quick Edit Student" class="actionicons" onClick="showStudentFrm('<?=$V->id?>', 'QF');" />
            <? if($actionPage!="Grade") { ?><img src="images/edit_icon.png" alt="Detailed Edit Student" title="Detailed Edit Student" class="actionicons" onClick="showStudentFrm('<?=$V->id?>', 'DF');" /><? } ?>
            <? if($actionPage!="Grade") { ?><img src="images/delete_icon.png" alt="Delete Student" title="Delete Subject" onclick="if(confirm('Are you sure want to delete the selected Student?')) studentActions(<?=$V->id?>, 'D')" class="actionicons" /><? } ?>
            </div>
            </td>
        </tr>
    <?
	...
	?>
    	<tr bgcolor="#FFFFFF">
            <td colspan="8" style="padding:10px;">
                ...
                <div class="combutton pull_left marginleft10" onclick="showNewStudentOption('<?=$gradeId?>');">MAKE AS NEW STUDENT</div>
            </td>
        </tr>
    <?
} else {
