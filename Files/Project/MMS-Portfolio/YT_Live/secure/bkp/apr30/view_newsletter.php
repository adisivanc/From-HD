...
if($_POST['circular_action']=="P") {
	...
	if($_POST['is_configure_url']=="Y") {
		if($_POST['button_type']=="I") {
			
			if($_FILES['button_image']['size']>0) {
				$button_file_name = "button".".".end(explode(".", $_FILES['button_image']['name']));
				$button_files = SITE_TMP_DIR.$button_file_name;
				if(is_file($button_files)) { unlink($button_files); }
				move_uploaded_file($_FILES['button_image']['tmp_name'], $button_files);	
			} 
			else {
				$button_file_name = $rs_circular->button_details;
				$source_button = NEWSLETTER_BUTTON_HREF;
				$destination_button = SITE_TMP_DIR;
				if (in_array($button_file_name, array(".",".."))) continue;
				copy($source_button.$button_file_name, $destination_button.$button_file_name);
			}
			
			$button_file = "button";
			if(is_file($button_file)) { unlink($button_file); }
			move_uploaded_file($_FILES['button_image']['tmp_name'], $button_file);
			$buttonImg = "<a href={SURL} target='_blank'><img src='".$ns_button_path.$button_details."' title='Update Details' alt='Update Details' border='0' /></a>";
			$welcome_text = str_replace("{BUTTON}", $buttonImg, $welcome_text);
			$welcome_description = str_replace("{BUTTON}", $buttonImg, $welcome_description);
		}
		if($button_type=="T") {
			$buttonImg = "<div align='center'><a href={SURL} target='_blank' style='color:#000000;'><b><u>".$button_details."</u></b></a></div>";
			$welcome_text = str_replace("{BUTTON}", $buttonImg, $welcome_text);
			$welcome_description = str_replace("{BUTTON}", $buttonImg, $welcome_description);
		}
	}
	...
	$ns_button_path = NEWSLETTER_BUTTON_HREF;
...
//if($fileFrom=="Mail") {
	$configNewURL = $configure_url;
	$configNewURL = str_replace('{STUDENT_ID}', base64_encode($s_or_t_id), $configNewURL);
	$configNewURL = str_replace('{GRADE_ID}', base64_encode($grade_id), $configNewURL);
	$configNewURL = str_replace('{EMAIL_ADDRESS}', base64_encode($emailAddress), $configNewURL);
	$configNewURL = str_replace('{teacher_id}', base64_encode($s_or_t_id), $configNewURL);
	$urlContent = $configNewURL;
	$welcome_text = str_replace('{SURL}', $urlContent, $welcome_text);
	$welcome_description = str_replace('{SURL}', $urlContent, $welcome_description);
					...
					<tbody>
						...
						<tr>
                            ...
                            <? if($header_type=="I" && $headerArr['Img']!="") { ?>
								<p style="max-width:800px;"><img alt="<?=stripslashes($title)?>" title="<?=stripslashes($title)?>" border="0" src="<?=$ns_header_path.$headerArr['Img']?>" style="width:100%; float:left; " /></p>
							<? } ?>
                            <? if($header_type=="T") { ?>
						...
						<tr>
							<td>
								<table width="745" border="0" cellspacing="0" cellpadding="0" style="margin:0 auto; padding:12px 0;">
                                  <tr>
                                    <td colspan="3">
                                        <? if($welcome_note!="") { ?><div><?=stripslashes($welcome_note)?></div><? } ?>
                                        <? if($welcome_text!="") { ?><br /><div style="line-height:22px; text-align:justify;"><?=stripslashes($welcome_text)?></div> <? } ?>
                                    </td>
                                  ...
                                  <tr>
                                    <td colspan="3"><div style="line-height:23px; padding:10px 0 3px 0; text-align:justify;"><?=stripslashes($welcome_description)?></div></td>
                                  </tr>
                                  <? } ?>
                                </table>
							</td>
                        ...
                        <tr>
                            <td>
                                <table border="0" cellpadding="0" cellspacing="0" width="745" style="margin:0 auto;">
                                    <tr>
                                        <td>
                                            <p style="line-height:23px; text-align:justify; margin:0; padding:5px 0;"><?=stripslashes($conclusion_text)?></p>
                                        </td>
                                    </tr>
                                </table>
                            </td>
                        ...
                        <tr>
                            ...
                            <table border="0" cellpadding="0" cellspacing="0" width="745" style="margin:0 auto;">
                                ...
                                <? if($regards_text!="") { ?><?=stripslashes($regards_text)?>,<? } ?> 
                                <? if($regards_from_text!="") { ?><br /> <?=stripslashes($regards_from_text)?>.<? } ?>
                                </p>
                                        </td>
                                    </tr>
                                </table>
