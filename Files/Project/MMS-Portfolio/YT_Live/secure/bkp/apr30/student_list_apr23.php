...
function showNewStudentOption(grade_id) {
	...
	ajax({
		a:'grade',
		b:'act=loadAllGradeStudents&studentIds='+studentIds+'&schoolId='+school_id+'&gradeSection='+grade_section+'&grade_id='+grade_id,		
		c:function(){},
		d:function(data){
			popupDtls(data);
		}			
...
function updateNewStudent(grade_id) {
	...
	if(err==0) {
		...
		ajax({
			a:'grade',
			b:'act=setNewStudents&gradeId='+grade_id+'&studentIds='+studentIds+'&schoolId='+school_id+'&gradeSection='+grade_section+'&newBatch='+new_batch,		
			c:function(){},
			d:function(data){ //alert(data); return false;
				alert('Updated Successfully..!');
				$('#processBtn').hide();
