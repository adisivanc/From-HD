...
if($_POST['act']=="saveCircular") {
	...
	if($_POST['newsletter_db_id']!="" && $_POST['newsletter_db_id']!="undefined") {
		...
		Circulars::updateCircular($rs_id, $ns_type, $title, $subject, $header_type, $welcome_note, $welcome_text, $no_of_inner_image, $welcome_description, $conclusion_text, $regards_text, $regards_from_text, $send_date, $send_time, $send_time_format, $send_date_position, $status, $apply_to, $is_highlight_text, $term_calender_position, $ns_date);
	} else {
		$rs_id = Circulars::insertCircular($ns_type, $title, $subject, $header_type, $welcome_note, $welcome_text, $no_of_inner_image, $welcome_description, $conclusion_text, $regards_text, $regards_from_text, $send_date, $send_time, $send_time_format, $send_date_position, $status, $apply_to, $is_highlight_text, $term_calender_position, $ns_date);
	}
    ...
    <table width="100%" border="0" style="background:#FFF;" class="popuptbl" cellpadding="0" cellspacing="0">
        ...
        <td id="circular_td">
        	<table>
                ...
                <tr>
                	<td>Send this newsletter to </td>
                    <td>
                    	<input type="radio" name="send_to_option" id="send_to_option" value="Y" /> New Students
                        <input type="radio" name="send_to_option" id="send_to_option" value="N" /> Old Students
                        <input type="radio" name="send_to_option" id="send_to_option" value="B" checked/> Both
                    </td>
                </tr>
				...
				if($rs_tt[0]=='SSG'){
					...
					if(count($rs_SelNLMember)>0) {
						foreach($rs_SelNLMember as $kk=>$vv) {
							...
							if($_POST['send_to_option']!="B" && $_POST['send_to_option']!="" && $_POST['send_to_option']!="undefined") $student_obj->is_new_student = $_POST['send_to_option'];
...
function bulkCircularForm() {
   ...
   err=0;
	...
	var send_to_option = $('input[name=send_to_option]:checked').val();
	
	if(err==0) {
		$('#circular_td').html('<span style="font-size:24px; padding:20px; color:#BD220A">sending please wait...</span>');
		ajax({
			a:'newsletter',							
			b:'act=sendCircularEmail&nid='+nid+'&from_address='+from_address+'&Subject='+Subject+'&circular_group='+circular_group+'&addschedule='+addschedule+'&send_unsub='+send_unsub+'&resend_not_receive='+resend_not_receive+'&send_to_option='+send_to_option,
			c:function(){},
			d:function(data) {
				//alert(data);
