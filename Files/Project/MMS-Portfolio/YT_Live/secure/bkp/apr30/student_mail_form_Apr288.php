...
if($_POST['act']=="sendCircularToStudent") {
	...
	if($sent_type=="S" && $student_id!="" && $student_id!="undefined") {
		if($circular_id=="N") {
			...
			if($rs_student->father_email!="" && $rs_student->f_email_subscription=="Y") $emailArr[]=$rs_student->father_email."~".$rs_student->father_name."~".$rs_student->father_phone."~"."SF"."~".$student_name."~".$rs_student->id."~".$rs_student->grade_id;
			if($rs_student->mother_email!="" && $rs_student->m_email_subscription=="Y") $emailArr[]=$rs_student->mother_email."~".$rs_student->mother_name."~".$rs_student->mother_phone."~"."SM"."~".$student_name."~".$rs_student->id."~".$rs_student->grade_id;
			if($rs_student->father_email=="" && $rs_student->mother_email=="" && $rs_student->e_email_subscription=="Y") 
			$emailArr[]=$rs_student->email_address."~".$rs_student->father_name."~".$rs_student->mobile."~"."SE"."~".$student_name."~".$rs_student->id."~".$rs_student->grade_id;
			...
			if($sent_type=="GS") { 
				...
				if(count($rs_SelNLMember)>0) {
					foreach($rs_SelNLMember as $kk=>$vv) {
						...
						$student_obj->fields = "id, first_name, middle_name, last_name, email_address, father_email, mother_email, father_phone, mother_phone, father_name, mother_name, mobile";
						$rs_student = $student_obj->getAllStudentDtls();
						$student_name = $rs_student->first_name." ".$rs_student->middle_name." ".$rs_student->last_name;
						if($rs_student->father_email!="") $emailArr[]=$rs_student->father_email."~".$rs_student->father_name."~".$rs_student->father_phone."~"."SF"."~".$student_name."~".$vv->student_id."~".$vv->grade_id;
						if($rs_student->mother_email!="") $emailArr[]=$rs_student->mother_email."~".$rs_student->mother_name."~".$rs_student->mother_phone."~"."SM"."~".$student_name."~".$vv->student_id."~".$vv->grade_id;
						if($rs_student->father_email=="" && $rs_student->mother_email=="" && $rs_student->email_address!="") $emailArr[]=$rs_student->email_address."~".$rs_student->father_name."~".$rs_student->mobile."~"."SE"."~".$student_name."~".$vv->student_id."~".$vv->grade_id;
					}
				}
			}
	...
	<tr>
        ...
        <td>
            <select name="send_circular_id" id="send_circular_id" class="listbox" onchange="showNewCircular()">
                ...
                <?
                if(!empty($rs_ciruclar) && count($rs_ciruclar)>0) { $listArr = array();
                    foreach($rs_ciruclar as $K=>$V) { 
					$applyArr = unserialize($V->apply_to); 
					$listArr = $applyArr[$nstype]["SS:".$schoolId];
					if(in_array("SSG:".$gradeId, $listArr)) {
...
function sendStudentMail() { 
	...
	if(err==0) { alert('act=sendCircularToStudent&circular_id='+circular_id+'&grade_id='+grade_id+'&school_id='+school_id+'&toEmail='+toEmail+'&subject='+subject+'&msg='+msg+'&sendType='+sendType+'&student_id='+student_id);
		$('#sendbtn').hide();
		$('#sendingbtn').show();
		ajax({
			a:'student_mail_form',
