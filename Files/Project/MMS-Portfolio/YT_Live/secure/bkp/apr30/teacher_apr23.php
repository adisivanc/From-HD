	...
	if(count($rs_teacher)>0) {
        ...
        <div class="teacherlist_inner cursor" id="teacher_deatils1">
            <div style="width:100%; height:160px; border:1px solid #999999;" onclick="showTeacherDtls('TD', '<?=$v->id?>', '<?=$v->teacher_status?>');">
            	<img src="<?=TEACHERS_FILE_HREF.$v->photo?>" alt="<?=$teacher_name?>" title="<?=$teacher_name?>" height="160px" width="100%" />
            </div>	
            <h4><?=$teacher_name?></h4>
            <h5><?=$subjects?></h5>
        </div>
        ...
        <tr>
            <td>
                ...
                if(count($rs_teacher)>0){
                    foreach($rs_teacher as $k=>$v){
						$rs_subject = Subject::getSubjectsByIds($v->subject_id);
						$rs_school = School::getSchoolById($v->school_id);
						$subjects = implode(", ", $rs_subject);
						$teacher_name = $v->prefix.".".$v->first_name." ".$v->middle_name." ".$v->last_name;
						$teacher_name = trim($teacher_name);
                    ...
                    ?>
                        <tr bgcolor="<?=$bgColor?>">
                            ...
                            <td align="center">
                            <div class="btn_group">
                            <img src="images/view_icon.png" alt="Edit Subject" title="Edit Subject" onclick="showTeacherDtls('TD', '<?=$v->id?>', '<?=$v->teacher_status?>', '<?=$_POST['listType']?>');" class="actionicons" /> 
                            <img src="images/edit_icon.png" alt="Edit Subject" title="Edit Subject" onClick="showTeacherDtls('E', '<?=$v->id?>', '<?=$v->teacher_status?>', '<?=$_POST['listType']?>')" class="actionicons" /> 
                            </div>
                            </td>
                        </tr>
                    <?
        ...
        <div class="fullsize newsletter_outer">
            ...
            <div class="newsletter_left"> <!-- Newsletter Circular -->
            	<div class="newsletter_submenu txtwhite">
                    ...
                    <div class="circular_outer" style="clear:both;">
                    	<div class="newcircular_head tabbtn" id="tabbtn_I" onclick="showTeacherDtls('TL', '', 'I');">Archive <span></span></div>
                        <ul class="listoption" id="teachermenutab_I" style="padding-right:10px;"></ul>
                    </div>
                </div>
...
function showTeacherDtls(type, id, status, list_type) {  //alert(type); alert(id); alert(status);
	...
	if(type=="TL") {
		...
		$("#teachermenutab_"+status).html('<div style="padding:50px;"><img src="images/loader.gif" alt="Loading Data.." title="Loading Data.." /></div>');
	}
...
function showTeacherListDtls(status, list_type) { 
	...
	var search_by = $.trim($('#search_by').val());
	$("#teacherListTab").html('<div style="padding:50px;"><img src="images/loader.gif" alt="Loading Data.." title="Loading Data.." /></div>');
