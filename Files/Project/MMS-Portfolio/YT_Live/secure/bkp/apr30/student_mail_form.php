			...
			if($sent_type=="GS") { 
				...
				if(count($rs_SelNLMember)>0) {
					foreach($rs_SelNLMember as $kk=>$vv) {
						...
						$student_name = $rs_student->first_name." ".$rs_student->middle_name." ".$rs_student->last_name;
						if($rs_student->father_email!="") $emailArr[]=$rs_student->father_email."~".$rs_student->father_name."~".$rs_student->father_phone."~"."SF"."~".trim($student_name)."~".$vv->student_id."~".$vv->grade_id;
						if($rs_student->mother_email!="") $emailArr[]=$rs_student->mother_email."~".$rs_student->mother_name."~".$rs_student->mother_phone."~"."SM"."~".trim($student_name)."~".$vv->student_id."~".$vv->grade_id;
						if($rs_student->father_email=="" && $rs_student->mother_email=="" && $rs_student->email_address!="") $emailArr[]=$rs_student->email_address."~".$rs_student->father_name."~".$rs_student->mobile."~"."SE"."~".trim($student_name)."~".$vv->student_id."~".$vv->grade_id;
					}
				}
			}
...
if($sendType=="GS") $nstype = "S"; else $nstype = $sendType;
