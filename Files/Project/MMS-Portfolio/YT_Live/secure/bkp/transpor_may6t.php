...
if($_POST['act']=="loadRouteStops") {
    ...
    <div class="fullsize">
        ...
        <div class="fullsize lineht2 border_bottom">
            ...
            <? if($rs_route->id!=NULL) { ?>
            <div class="pull_right padlr10 padtb10 txtbold letterspac f18"><img src="images/edit_icon.png" alt="Edit" title="Edit" onClick="showTransFrm('B', '<?=$_POST['routeId']?>');" align="absmiddle" style="cursor:pointer;" /> </div>
            <div class="pull_right padlr10 padtb10 txtbold letterspac f18"><img src="images/delete_icon.png" alt="Delete" title="Delete" onClick="if(confirm('Are you sure want to delete the Bus Route?')) deleteTransFrm('B', '<?=$_POST['routeId']?>');" align="absmiddle" style="cursor:pointer;" /></div>
			...
			<div class="pull_right combutton margin5 txtbold letterspac f18" onclick="showAllStudents('<?=$_POST['routeId']?>')">Assign Student</div>
            <? } ?>
        </div>
...
if($_POST['act']=="loadStopTypeDtls") {
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="gradetbl" id="stopTbl">
      ...
      <tr id="<?=$V->id?>" style="cursor:move;" class="dragTr">
        ...
        <td class="dragHandle">
            <img src="images/student_icon.jpg" alt="Add Student" title="Add Student" onClick="showStudentsFrm('<?=$V->id?>', '<?=$_POST['routeId']?>', '<?=$_POST['stopType']?>')" align="absmiddle" style="cursor:pointer;" width="25" height="25" />
        </td>
      </tr>
      <?
          }
...
if($_POST['act']=="loadStopAssigningFrm") {
	...
	function defaultLoader(){
		...
		if((!empty($studentStopsArr)) &&(count($studentStopsArr)>0)){
			...
			?>
				addStudent({id:'<?=$tv->student_id?>', stop_student_name:'<?=$student_name?>'});
		<?  } 
		} 
		else { ?>addStudent(); <? } ?>
	}
	...
	function addStudent(a){
		...
		vhtml += '			<td style="padding:5px;"><input type="text" class="txtbox" name="StudentStopArr[stop_student_name]['+row+']" id="stop_student_name'+row+'" value="'+$.trim(a.stop_student_name)+'" placeholder="Type Student Name" /><input type="hidden" class="stop_student_id" name="StudentStopArr[stop_student_id]['+row+']" id="stop_student_id'+row+'" value="'+a.id+'" /></td>';
