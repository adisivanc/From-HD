...
if($_REQUEST['search_type']=="student") {
	...
	if($_REQUEST['school_id']!='' && $_REQUEST['school_id']!="undefined") $student_obj->school_id=$_REQUEST['school_id'];
	if($_REQUEST['grade_id']!='' && $_REQUEST['grade_id']!="undefined") $student_obj->grade_id=$_REQUEST['grade_id'];
	if($_REQUEST['student_notin_route']!='' && $_REQUEST['student_notin_route']!="undefined") $student_obj->student_notin_route=$_REQUEST['student_notin_route'];
...
if($_REQUEST['search_type']=="bus_student") {
	$student_obj = new Student();  
	$student_obj->name_search=$q;
	if($_REQUEST['school_id']!='' && $_REQUEST['school_id']!="undefined") $student_obj->school_id=$_REQUEST['school_id'];
	if($_REQUEST['student_notin_route']!='' && $_REQUEST['student_notin_route']!="undefined") $student_obj->student_not_in_route=$_REQUEST['student_notin_route'];
	$search_rs = $student_obj->getAllStudentDtls();
	if(count($search_rs)>0) {
		 foreach($search_rs as $k=>$v) {
			$student_name = $v->first_name." ".$v->middle_name." ".$v->last_name;
