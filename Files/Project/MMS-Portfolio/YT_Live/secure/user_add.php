...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="listtbl">
    ...
    <tr>
        ...
        <td>
        <input type="radio" name="user_type" id="user_type" value="SA" <? if($rsUserName->user_type=='SA') echo "checked='checked'"?> onchange=""/>Super Admin
        <input type="radio" name="user_type" id="user_type" value="A" <? if($rsUserName->user_type=='A') echo "checked='checked'"?> />Admin
        <input type="radio" name="user_type" id="user_type" value="FO" <? if($rsUserName->user_type=='FO' ) echo "checked='checked'"?> />Front Office
        <input type="radio" name="user_type" id="user_type" value="T" <? if($rsUserName->user_type=='T' || $rsUserName->user_type=='' ) echo "checked='checked'"?> />Teacher
        </td>
    </tr>
 ...
 <!--   <tr>
        ...
        <td>
            ...
            ?>
            <input type="checkbox" class="school_id" id="school_id<?=$index1?>"  name="school_id" value="<?=$V->id?>" onclick="showAccessType('<?=$V->id?>', 'S'); " <? if(in_array($V->id, $SchoolArr)) { echo "checked"; } ?> /> <?=$rs_school->school_name?>
            <? 
            }
            ?>
  ...
  ---------->  
    ...
    <tr>
        <td colspan="2" style="">
            ...
            <h3 style="font-weight:bold; padding-bottom:5px;">School/Grade Access</h3>
            <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background:#dab791;">
              <tr>
                <td width="25%" valign="top" id="showSchlError">Select School</td>
                <td valign="top">
                <?
            $schoolObj = new School();
            $rsSchool = $schoolObj->getSchoolDtls();  
			...
			$SchoolArr = explode(",",$rsUserName->school_id);
            ?>
                	<select class="multi_selectbox" id="school_id" name="school_id" multiple="multiple" style="width:70%;" onchange="showAccessType(this.value, 'S'); showGradeList();">
                    <option value="All" <? if(in_array("All", $SchoolArr)) { echo "selected"; } ?>>All</option>
                    	<? 
						 foreach($rsSchool as $K=>$V) { $rs_school = School::getSchoolById($V->id);
                         ?>
                    	<option name="school_id" value="<?=$rs_school->id?>"  <? if(in_array($V->id, $SchoolArr)) { echo "selected"; } ?> ><?=$rs_school->school_name?></option>
                        <?
						 }
						?>
              ...
              <tr id="gradeleveltab">
                <td valign="top" id="showGradeLvlError">Grade Level</td>
                <td valign="top"  id="gradelevellist">
    ...
    <tr>
        <td colspan="2" style="">
        	<? $acc_Arr=explode(',',$rsUserName->access_control);?>
            <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:5px; float:left; background:#dab791;">
              <tr>
                <td width="25%" valign="top">Blog Access</td>
                <td valign="top">
					<input type="checkbox" class="check_lg" id="acc_control_b" name="acc_control" value="B" <? if(in_array('B',$acc_Arr)){?> checked="checked"<? } ?>/>
                </td>
              </tr>
            </table>
    ...
    <tr>
        <td colspan="2" style="">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:5px; float:left; background:#dab791;">
              <tr>
                <td width="25%" valign="top">Event Access Control</td>
                <td valign="top">
					<input type="checkbox" class="acc_control" id="acc_control_e" name="acc_control" value="E" <? if(in_array('E',$acc_Arr)){?> checked="checked"<? } ?> />
                </td>
              </tr>
            </table>
    ...
    <tr>
        <td colspan="2" style="">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:5px; float:left; background:#dab791;">
              <tr>
                <td width="25%" valign="top">Newsletter</td>
                <td valign="top">
					<input type="checkbox" class="check_lg" id="acc_control_n" name="acc_control" value="N" <? if(in_array('N',$acc_Arr)){?> checked="checked"<? } ?>/>
                </td>
              </tr>
            </table>
	...
	<tr>
    	<td>Visible To Parent Portal</td>
        <td>
        	<input type="radio" name="parent_portal" id="parent_portal_yes" value="Y"<? if($rsUserName->user_type==''){?>checked="checked"<? }if($rsUserName->user_type!=''){?><? if($rsUserName->parent_portal=="Y"){?>checked="checked"<? }}?>/>Yes
           <input type="radio" name="parent_portal" id="parent_portal_no" <? if($rsUserName->user_type=='T'){?>disabled="disabled" <? }else{ if($rsUserName->parent_portal=='N'){?>checked="checked"<? }} if($rsUserName->user_type==''){ ?> disabled="disabled" <? }?>/>No
        </td>
    </tr> 
    <tr>
...
<script type="text/javascript">
 $("input:radio[name=user_type]").click(function(){
	...
	var type=$("input:radio[name=user_type]:checked").val();
		if(type!='T')
		{
			$("input:radio[id=parent_portal_no]").removeAttr('disabled','disabled');
		}
		else{
			$("input:radio[id=parent_portal_yes]").attr('checked', true);
			$("input:radio[id=parent_portal_no]").attr('disabled','disabled');
			}
...
function showGradeList(gradeIds) {
	...
	var school_ids=$('#school_id :selected').val();
	$('#gradeleveltab').hide();
	$('#gradelevellist').html(''); 
	if(school_ids!="") {
