            ...
            <?
			  	if(count($rs_usersArr)>0) {
					foreach($rs_usersArr as $K=>$V) {
						...
						if($V->grade_level=="All"||$V->grade_level=="All,") { $gradeLevels = "All";
						} else {
						...
						$is_log = UserLog::checkLogExistsById(TBL_USERS, $V->id);
			  ?>
                      <tr>
                        ...
                        <td align="center">
                            <div class="btn_group">
                            <? if($GLOBALS['isUpdate']){ ?><img src="images/edit_icon.png" alt="Edit User" title="Edit User" onclick="showUsersPopup(<?=$V->id;?>)" class="actionicons" /><? } ?>
                            <? if($GLOBALS['isDelete']){ ?><img src="images/delete_icon.png" alt="Delete User" title="Delete User" onclick="if(confirm('Are you sure want to delete this User?')) deleteUser('<?=$V->id?>')" class="actionicons" /><? } ?>
                            <? if($_SESSION['viewLog'] && $is_log==1) { ?><img src="images/log.png" alt="Log Details" title="Log Details" onClick="showLogDetails('<?=TBL_USERS?>', '<?=$V->id?>')" align="absmiddle" style="cursor:pointer;" /><? } ?>
                            </div>
                        </td>
                      </tr>
              <?
...
if($_POST['act']=='saveUserFrm'){
	...
	$gArr = explode(",", $_POST['grade_level']);
	$gradeArr = array(); 
	...
	/*
	id 	school_id 	user_type 	name 	access_control 	email_address 	phone 	user_name 	password 	access_type 	grade_level 	status
	*/
	
	$postArrs=array('id','school_id','user_type','name','access_control','email_address','phone','user_name','password','access_type','grade_level','parent_portal');
				$stringArr = array('name','user_name');
				foreach($postArrs as $K=>$V) 
				{
					if($V=="school_id"){$$V=$schoolid;	}					
					if($V=="grade_level"){$$V=$gradeids;}
					if($V!="school_id" && $V!="school_id"){$$V=$_POST[$V];}
	...
	if($argV['id']!=''){
		User::updateUserTable($argV);
	}else{ 
		User::insertUserTable($argV);
	}
exit();	
}
...
if($_POST['act']=="showGradeListDtls") {
	...
	?>
    <select name="grade_ids" multiple="multiple" style="width:70%;" id="grade_ids" onchange="showAccessType(this.value, 'G');">
   			<option value="All" <? if(in_array("All", $GradeArr)) { echo "selected"; } ?> >ALL</option> 
            <?
			if(count($rs_grades)>0)
             foreach($rs_grades as $K=>$V) { $rs_grade = Grade::getGradeById($V->id);
			 ?>
            <option value="<?=$V->id?>" <? if(in_array($V->id, $GradeArr)) { echo "selected"; } ?>><?=$rs_grade->grade_name?></option>
            <?
			 }
			?>
    </select>
...
<div class="fullsize">
    ...
    <div class="fullsize menu_head padtb10">
        ...
        <div class="pull_right f24 padtop50 cursor" id="show_addteacher">
            <? if($_SESSION['viewLog']) { ?><div class="combutton" onclick="showLogDetails('<?=TBL_USERS?>', '')">Logs</div><? } ?>
        </div>
    </div>
    ...
    <div class="fullsize padtb15">
        ...
        <div class="newsletter_right border_theme bgwhite" id="hide_allgradelist" style="width:79%;"> <!-- Grade -->
        	...
        	<div class="fullsize lineht2 border_bottom">
                ...
                <? if($GLOBALS['isAdd']){ ?><div class="combutton pull_right" onclick="showUsersPopup('')" style="clear:both;">Add Users</div><? } ?>
            </div>
...
function submitUser(user_id){ 
	...
	if($.trim($('#school_id :selected').val())==''){ err=1; $('#showSchlError').addClass('txterror'); } else{ $('#showSchlError').removeClass('txterror'); }
	if($.trim($('#grade_ids').val())==''){ err=1; $('#showGradeLvlError').addClass('txterror'); } else{ $('#showGradeLvlError').removeClass('txterror'); }
	 
	
	var accessControl = new Array();
	if($.trim($('input[name=acc_control]:checked').val())==''){ $('#showError').addClass('txterror'); } else{ $('#showError').removeClass('txterror'); $('input[name="acc_control"]:checked').each(function() {
		accessControl.push(this.value);
	}); }
	...
	$('input[name="access_type"]:checked').each(function() {
	 ...
	 var gradeIds = "";
        $( "#grade_ids option:selected" ).each(function() {
          gradeIds += $( this ).val() + ",";
        });
	var schoolIds = $('#school_id option:selected').val();
	
	var parent_portal=$("input:radio[name=parent_portal]:checked").val();
	...
	if(err==0){
		...
		ajax({
			a:'users',
			b:'act=saveUserFrm&school_id='+schoolIds+'&name='+$('#name').val()+'&email_address='+$('#email_address').val()+'&phone='+$('#phone').val()+'&user_name='+$('#user_name').val()+'&password='+$('#password').val()+'&user_type='+$('input[name=user_type]:checked').val()+'&access_type='+accessType+'&id='+user_id+'&grade_level='+gradeIds+'&access_control='+accessControl+'&parent_portal='+parent_portal,		
			c:function(){},
			d:function(data){
