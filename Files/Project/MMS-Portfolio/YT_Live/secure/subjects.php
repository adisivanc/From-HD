            ...
            if(count($rsSubjectArr)>0){
				...
				?>
					<tr bgcolor="<?=$bgColor?>">
						...
						<td align="center">
						<div class="btn_group">
						<? if($GLOBALS['isUpdate']){ ?><img src="images/edit_icon.png" alt="Edit Subject" title="Edit Subject" onclick="showSubjectsPopup(<?=$V->id;?>)" class="actionicons" /><? } ?> 
						<? if($GLOBALS['isDelete']){ ?><img src="images/delete_icon.png" alt="Delete Subject" title="Delete Subject" onclick="if(confirm('Are you sure want to delete the selected Subject?')) deleteSubject(<?=$V->id;?>, '<?=$_POST['school_id']?>')" class="actionicons" /><? } ?>
						</div>
						</td>
					</tr>
				<?
...
if($_POST['act']=='saveSubjectsFrm'){
	...
	if($_POST['subject_id']!=''){
		$newIdsArr = explode(",", $gradeIds);
		$rs_subject = Subject::getSubjectById($subject_id);
		$oldGradeIdArr = explode(",", $rs_subject->grade_id);
		foreach($oldGradeIdArr as $k=>$v) {
			if(!in_array($v, $newIdsArr)) Teacher::unsetGradeTeacherBySubjectId($subject_id, $school_id, $v);
		}
		$rsSubjectUpd = Subject::updateSubject($school_id, $gradeIds, check_input(ucwords($subject_name)), check_input($description), $subject_id);
	} else {
		$rsSubjectIns = Subject::insertSubject($school_id, $gradeIds, check_input(ucwords($subject_name)), check_input($description));
	}
	exit();
}
...
if($_POST['act']=='delSubjects'){
	ob_clean();
	Subject::deleteGradeSubjectBySubjectId($_POST['subject_id']);
	Teacher::unsetGradeTeacherBySubjectId($_POST['subject_id'], $_POST['school_id'], '');
	Subject::deleteSubject($_POST['subject_id']);
	exit();
}
...
<div class="fullsize">
    ...
    <div class="fullsize menu_head padtb10">
        ...
        <div class="pull_right f24 padtop50 cursor" id="show_addteacher">
            <? if($_SESSION['viewLog']) { ?><div class="combutton" onclick="showLogDetails('<?=TBL_SUBJECTS?>', '')">Logs</div><? } ?>
        </div>
    </div>
    ...
    <div class="fullsize padtb15">
        ...
        <div class="newsletter_right border_theme bgwhite" id="hide_allgradelist" style="width:79%;"> <!-- Grade -->
        	...
        	<div class="fullsize lineht2 border_bottom">
                ...
                <? if($GLOBALS['isAdd']){ ?><div class="combutton pull_right" onclick="showSubjectsPopup('')" style="clear:both;">Add Subject</div><? } ?>
            </div>
...
function submitSubject(subject_id) { 
	...
	var subject_name = escape($.trim($('#subject_name').val()));
	...
	if(err==0){
		...
		ajax({
			a:'subjects',
			b:'act=saveSubjectsFrm&subject_name='+subject_name+'&description='+description+'&subject_id='+subject_id+'&school_id='+school_id+'&subject_grades='+subject_grades,		
			c:function(){},
...
function deleteSubject(subject_id, school_id) { 
	ajax({
		a:'subjects',
		b:'act=delSubjects&subject_id='+subject_id+'&school_id='+school_id,		
		c:function(){},
		d:function(data) { //alert(data); return false;
