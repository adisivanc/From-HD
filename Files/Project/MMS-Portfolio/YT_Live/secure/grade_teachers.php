...
<div class="gradeteacher_list">
    <img src="resize.php?w=250&h=160&img=<?="../".TEACHERS_FILE_REL.$rs_teacher->photo?>" alt="<?=$rs_teacher->teacher_name?>" title="<?=$rs_teacher->teacher_name?>" /><br />
    <div style="width:75%; padding:5px; margin-left:19px;">
    	<? if($GLOBALS['isUpdate']){ ?><img src="images/edit_icon.png" alt="Edit" title="Edit" align="absmiddle" class="cursor" onclick="showTeacherFrm('E', '<?=$gradeId?>', '<?=$V->id?>', '<?=$V->teacher_id?>')" /><? } ?>
        <? if($GLOBALS['isDelete']){ ?>
