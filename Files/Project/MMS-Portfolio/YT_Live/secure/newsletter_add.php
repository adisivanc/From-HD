...
<div class="fullsize_pad padtb10 lineht1_8">
    ...
    <div class="pull_left marginright20">Newsletter Type
...
<div class="newsletter_cntr">
    ...
    <div class="fullsize bgtheme"><? $rs_ApplyTo = unserialize($rs_circular->apply_to); ?>
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="moduletbl">
          <tr>
          ...
          <tr>
            ...
            <td style="line-height:25px; padding:0px;" valign="top">
						...
						foreach($rs_ApplyTo as $k1=>$v1) {
							...
							if(is_array($v1)) {
								foreach($v1 as $k2=>$v2) {
									$schlArr[] = $k2;
									if(is_array($v2)) {
										foreach($v2 as $k3=>$v3) $gradeArr[] = $v3;
									}
								}
							}
						}
					}
					//echo "<pre>"; print_r($rs_ApplyTo); echo "</pre>";
				?>
                <table border="0" cellpadding="0" cellspacing="0" width="100%">
                    <tr>
                        <td>
                        <h4><b><input type="checkbox" name="ApplyTo[T]" id="" class="parent_T" value="T" onclick="uncheckList('T', '', 'P')" <? if(in_array('T',$title)) { echo 'checked'; } ?>/> Teachers</b></h4>
                        <div style="margin-left:10px;" id="teacher_schl_list">
                            <?	
                                $sch_obj = new School();
								$rs_school = $sch_obj->getSchoolDtls();
                                $rs_schools = $rs_school;
                                
                                if(count($rs_schools)>0) {
                                    foreach($rs_schools as $K=>$V) {
                                    ?>
                                        <input type="checkbox" name="ApplyTo[T][TS:<?=$V->id?>]" id="sub_parent_T_<?=$K?>" class="parent_child_T" onclick="uncheckList('T', '<?=$K?>', 'C')" value="TS:<?=$V->id?>" <? if(in_array("TS:".$V->id,$schlArr)) { echo 'checked'; } ?>/> <b><?=$V->school_name?></b>
                                        <?
                                        $grd_obj = new Grade();
                                        $grd_obj->school_id = $V->id;
                                        $rs_grades = $grd_obj->getGradeDtls();
                                        ?>
                                        <div style="margin-left:10px;" id="teacher_grade_list">
                                        <?
                                        if(count($rs_grades)>0) {
                                            foreach($rs_grades as $kk=>$vv) {
                                            ?>
                                                <input type="checkbox" name="ApplyTo[T][TS:<?=$V->id?>][]" id="" class="child_T_<?=$K?> child_T" value="TSG:<?=$vv->id?>" <? if(in_array("TSG:".$vv->id,$gradeArr)){ echo 'checked'; } ?>/> <?=$vv->grade_name?><br />
                                            <?
                                            }
                                        }
                                        ?>
                        ...
                        <td style="border-left:1px solid #DAB791">
                        <h4><b><input type="checkbox" name="ApplyTo[S]" id="" class="parent_S" value="S" onclick="uncheckList('S', '', 'P')" <? if(in_array('S',$title)){ echo 'checked'; } ?>/> Students</b></h4>
                        <div style="margin-left:10px;" id="teacher_schl_list">
                            <?	
                                if(count($rs_schools)>0) {
                                    foreach($rs_schools as $K=>$V) {
                                    ?>
                                        <input type="checkbox" name="ApplyTo[S][SS:<?=$V->id?>]" id="sub_parent_S_<?=$K?>" class="parent_child_S" onclick="uncheckList('S', '<?=$K?>', 'C')" value="SS:<?=$V->id?>" <? if(in_array("SS:".$V->id,$schlArr)){ echo 'checked'; } ?>/> <b><?=$V->school_name?></b>
                                        <?
                                        $grd_obj = new Grade();
                                        $grd_obj->school_id = $V->id;
                                        $rs_grades = $grd_obj->getGradeDtls();
                                        ...
                                        if(count($rs_grades)>0) {
                                            foreach($rs_grades as $kk=>$vv) {
                                            ?>
                                                <input type="checkbox" name="ApplyTo[S][SS:<?=$V->id?>][]" id="" class="child_S_<?=$K?> child_S" value="SSG:<?=$vv->id?>" <? if(in_array("SSG:".$vv->id,$gradeArr)){ echo 'checked'; } ?>/> <?=$vv->grade_name?><br />
                                            <?
                                            }
                                        }
                                        ?>
                    ...
                    <tr>
                    	<td valign="top" colspan="2" style="border-top:1px solid #DAB791">
                        <h4><b><input type="checkbox" name="ApplyTo[NS]" id="subscriber_list" class="parent_NS" value="NS" <? if(in_array('NS',$title)){ echo 'checked'; } ?>/> Subscribers</b></h4>
                        </td>
                    </tr>
          ...
          <tr>
            ...
            <td>
                <input type="radio" class="" id="send_date_position" name="send_date_position" value="Y" onclick="showSendDateDtl()" <?=($rs_circular->send_date_position=="Y")?"checked":""?> /> Yes
                <input type="radio" class="marginleft20" id="send_date_position" name="send_date_position" onclick="showSendDateDtl()" value="N" <?=($rs_circular->send_date_position=="N" || $rs_circular->send_date_position=="")?"checked":""?> /> No
            </td>
          </tr>
          <tr class="senddatetab" style="display:none;">
            <td width="30%" class="txtright" valign="top"><span class="marginright10">Select Date</span></td>
            <td style="position:relative;">
                <input type="text" style="width:50%;" class="txtbox datepicker" id="send_date" name="send_date" value="<?=$rs_circular->send_date?>" />
                <img src="images/cal_icon.png" alt="" style="position:absolute; right:49%; bottom:11px;" />
            </td>
          </tr>
          <tr class="senddatetab" style="display:none;">
            <td width="30%" class="txtright" valign="top"><span class="marginright10">Set Send Time</span></td>
            <td>
            	<? $sendTimeArr = explode(":", $rs_circular->send_time); $sendTimeArr1 = explode(" ", $sendTimeArr[1]); ?>
            	<select class="listbox border_white" style="width:57px" id="send_time_hh" name="send_time_hh">
                    <option value="">HH</option>
                    <?  for($i=1; $i<=12; $i++){ ?>
                    <option value="<?=($i<10)?'0'.$i:$i?>" <?=($sendTimeArr[0]==$i)?"selected":""?>><?=($i<10)?'0'.$i:$i?></option>
                     <? } ?>
                </select>
                <span> : </span>
                <select class="listbox border_white" style="width:57px" id="send_time_mm" name="send_time_mm">
                    <option value="">MM</option>
                    <?  for($i=0; $i<=60; $i++){ ?>
                    <option value="<?=($i<10)?'0'.$i:$i?>" <?=($sendTimeArr1[0]!="" && $flightArr1[0]==$i)?"selected":""?>><?=($i<10)?'0'.$i:$i?></option>
                     <? } ?>
                </select>
                <input type="radio" name="send_time_ampm" id="send_time_am" value="AM" <?=($sendTimeArr1[1]=="AM" || $sendTimeArr1[1]=="")?"checked":""?> /> AM
                <input type="radio" name="send_time_ampm" id="send_time_pm" value="PM" <?=($sendTimeArr1[1]=="PM")?"checked":""?> /> PM
            </td>
          </tr>
          <tr>
            <td width="30%" class="txtright" valign="top"><span class="marginright10" id="sendtoerror">Send this newsletter to </span></td>
            <td>
            	<input type="radio" name="send_to_option" id="send_to_option1" value="N" <?=($rs_circular->circular_send_to=="N")?"checked":""?> /> New Students
                <input type="radio" name="send_to_option" id="send_to_option2" value="O" <?=($rs_circular->circular_send_to=="O")?"checked":""?> /> Old Students
                <input type="radio" name="send_to_option" id="send_to_option3" value="A" <?=($rs_circular->circular_send_to=="A")?"checked":""?> /> All Students
            </td>
          </tr>
          <tr>
            <td width="30%" class="txtright" valign="top"><span class="marginright10">Is Configure URL</span></td>
            <td>
            	<input type="radio" name="is_configure_url" id="is_configure_url1" value="Y" onclick="showConfigureDtls()" <?=($rs_circular->is_configure_url=="Y")?"checked":""?> /> Yes
                <input type="radio" name="is_configure_url" id="is_configure_url2" value="N" onclick="showConfigureDtls()" <?=($rs_circular->is_configure_url=="N" || $rs_circular->is_configure_url=="")?"checked":""?> /> No
            </td>
          </tr>
          <tr class="configureurloption" style="display:none;">
            <td width="30%" class="txtright" valign="top"><span class="marginright10">Button</span></td>
            <td>
            	<input type="radio" name="button_type" id="button_type1" value="I" onclick="showConfigureBtn()" <?=($rs_circular->button_type=="I")?"checked":""?> /> Image
                <input type="radio" name="button_type" id="button_type2" value="T" onclick="showConfigureBtn()" <?=($rs_circular->button_type=="T")?"checked":""?> /> Text
            </td>
          </tr>
          <tr id="buttonoption_I" class="buttonoption" style="display:none;">
            <td width="30%" class="txtright" valign="top"><span class="marginright10">&nbsp;</span></td>
            <td>
            	<input type="file" name="button_image" id="button_image" value="<?=$rs_circular->button_details?>" />
                <input type="hidden" name="h_button_image" id="h_button_image" value="<?=$rs_circular->button_details?>" />
            </td>
          </tr>
          <tr id="buttonoption_T" class="buttonoption" style="display:none;">
            <td width="30%" class="txtright" valign="top"><span class="marginright10">&nbsp;</span></td>
            <td>
            	<input type="text" name="button_text" id="button_text" value="<?=$rs_circular->buttom_details?>" class="txtbox" />
            </td>
          </tr>
          <tr class="configureurloption" style="display:none;">
            <td width="30%" class="txtright" valign="top"><span class="marginright10">Configure URL</span></td>
            <td>
            	<input type="text" name="configure_url" id="configure_url" class="txtbox" value="<?=$rs_circular->configure_url?>" style="width:98%;" />
                <br /><span style="font-size:12px;">ex:(http://www.yellowtrainschool.com/uniform.php?student_id={STUDENT_ID}&amp;grade_id={GRADE_ID}&amp;email_address={EMAIL_ADDRESS}&amp;teacher_id={TEACHER_ID})</span>
            </td>
          </tr>
        </table>
    </div>
    ...
    <div class="fullsize txtwhite txtcenter f18">
        ...
        <? if($rs_circular->status=="D" || $rs_circular->status=="") { ?><div class="bgbrown pull_right marginleft20 margintb10 cursor padlr20 padtb10" onclick="submitNSForm('D');"><strong>SAVE TO DRAFT</strong></div> <? } ?>
        <? } ?>
...
function submitNSForm(action) { 
	...
	if($('input[id="ns_header_type"]:checked').val()=='T'){ 
		...
		//if($('#ns_header_text2').val()==''){ err=1; $('#ns_header_text2').addClass('boxerror'); } else { $('#ns_header_text2').removeClass('boxerror'); }
		//if($('#ns_header_description').val()==''){ err=1; $('#headerdesctxt').addClass('txterror'); } else { $('#headerdesctxt').removeClass('txterror'); }
	...
	//if($('#welcome_note').val()==''){ err=1; $('#welcome_note').addClass('boxerror'); } else { $('#welcome_note').removeClass('boxerror'); }
	...
	if(nsType=="TE") {
		...
		for(var i=0; i<moduleCount; i++) {
			//if($('#module_title'+i).val()==''){ err=1; $('#module_title'+i).addClass('boxerror'); } else { $('#module_title'+i).removeClass('boxerror'); }
			//if($('#module_sub_title'+i).val()==''){ err=1; $('#module_sub_title'+i).addClass('boxerror'); } else { $('#module_sub_title'+i).removeClass('boxerror'); }
			//if($('#module_description'+i).val()==''){ err=1; $('#modultedesc'+i).addClass('txterror'); } else { $('#modultedesc'+i).removeClass('txterror'); }
			...
			if($('input[id="module_highlight_box'+i+'"]:checked').val()=="T") { 
				...
				//if($('#module_highlight_text_desc'+i).val()==''){ err=1; $('#module_highlight_text_desc'+i).addClass('boxerror'); } else { $('#module_highlight_text_desc'+i).removeClass('boxerror'); }
			}
	...
	if($('input[name="send_date_position"]:checked').val()=="Y") { 
		if($('#send_date').val()==''){ err=1; $('#send_date').addClass('boxerror'); } else { $('#send_date').removeClass('boxerror'); }
		if($('#send_time_hh').val()==''){ err=1; $('#send_time_hh').addClass('boxerror'); } else { $('#send_time_hh').removeClass('boxerror'); }
		if($('#send_time_mm').val()==''){ err=1; $('#send_time_mm').addClass('boxerror'); } else { $('#send_time_mm').removeClass('boxerror'); }
	}
	...
	if($('input[name=send_to_option]:checked').val()=='' || $('input[name=send_to_option]:checked').val()==undefined){ err=1; $('#sendtoerror').addClass('txterror'); } else { $('#sendtoerror').removeClass('txterror'); }
	
	var configure_url = $('input[name=is_configure_url]:checked').val();
	if(configure_url=="Y") {
		if($('#configure_url').val()==''){ err=1; $('#configure_url').addClass('boxerror'); } else { $('#configure_url').removeClass('boxerror'); }
	}
