        ...
        <tr>
            <td colspan="2" width="100%">
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                    	<td id="is_epload_err">Upload Files</td>
                         <td colspan="2">
                    <!--
...
if($_POST['act']=='saveFileFrm'){
	...
	if($_POST['file_id']!='' && $gallery_upload){
...
function submitFile(file_id) { 
	...
	if($('#attachment').val==""){ err =1; $('#is_epload_err').addClass('txterror');}else{  $('#is_epload_err').removeClass('txterror');}
	...
	if(err==0){
		...
		$.ajax({
			...
			success: function(data)  		// A function to be called if request succeeds
			{
				...
				if(result=="Uploaded Sucessfully")
				{
				closeFilePopup();
				 return false;
				$('#savebtn').show();
				...
				showFileDtls(school_id);
						 ...
						 $("#is_epload_err").addClass("txterror"); 
					}
			}	        
		});
