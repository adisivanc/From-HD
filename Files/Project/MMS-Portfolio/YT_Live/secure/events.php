...
if($_POST['act']=="eventSubmit") {
	...
	if($event_db_id != "" && $event_db_id != "undefined") {
		$rs_event_id = Events::updateEvents($event_db_id, $event_name, check_input($description), $event_type, $event_from_date, $event_to_date, $from_time, $to_time, $event_place, $event_address, $contact_person, $contact_phone, $contact_email, 'R');
		$rs_event_id = $event_db_id;
	} else {
		$rs_event_id = Events::insertEvents($event_name, check_input($description), $event_type, $event_from_date, $event_to_date, $from_time, $to_time, $event_place, $event_address, $contact_person, $contact_phone, $contact_email, $event_file, 'R');
	}
...
if($_POST['act']=="evtGalleryFrm") {
	...
	if($event_db_id != "" && $event_db_id != "undefined") { 
		$rs_event_id = Events::updateEvents($event_db_id, $event_name, addslashes($description), "", $event_from_date, "", "", "", "", "", "", "", "", 'G');
		$rs_event_id = $event_db_id;
	} else { 
		$rs_event_id = Events::insertEvents($event_name, addslashes($description), "", $event_from_date, "", "", "", "", "", "", "", "", "", 'G');
	}
...
<div class="fullsize">
    ...
    <div class="fullsize menu_head padtb10">
        ...
        <div class="pull_right f24 padtop40">
        	<? if($_SESSION['viewLog']) { ?><div class="combutton pull_right f24 cursor"><span onclick="showLogDetails('<?=TBL_EVENTS?>', '')" class="cursor">Logs</span></div><? } ?>
            <div class="combutton pull_right f24 cursor marginright10"><span onclick="showEventTabs('events', '', 'N')" class="cursor">Add Events</span></div>
        </div>
    </div>
