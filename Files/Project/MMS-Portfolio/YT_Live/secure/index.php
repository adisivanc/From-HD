...
{ 
	...
	if($_POST['password']!='' && $_POST['user_name']!=''){
		...
		if($rsChkDtls->id>0) {
			...
			$updatedData=''; 
			UserLog::updateUserLogDtls(array('tableId'=>$rsChkDtls->id, 'tableName'=>TBL_USERS, 'tableMasterId1'=>$rsChkDtls->id, 'userId'=>$_SESSION['YTUserId'], 'userType'=>$_SESSION['YTUserType']), $_SESSION['YTUserName'], $updatedData, "Login", "", "User ".trim($_SESSION['YTUserName'])." (#".$_SESSION['YTUserId'].") Logged In. ");
			
			$_SESSION['viewLog']=false;
			if($_SESSION['YTUserType']=="SA") {
				$_SESSION['viewLog'] = true;
			}
