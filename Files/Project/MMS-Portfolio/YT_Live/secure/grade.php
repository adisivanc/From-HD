...
?>
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="popuptbl">
        ...
        <tr>
        	<td>Send Email</td>
            <td><? $rs_ciruclars = Circulars::getMailTemplates($GLOBALS['MailTemplateCategoryTypes']['Student']); ?>
            	<select name="grade_circular_id[]" id="grade_circular_id" class="listbox" multiple="multiple" style="width:98%; height:130px;">
	...
	if($_POST['studentIds']!="" && $_POST['studentIds']!="undefined" && $_POST['newGradeId']!="" && $_POST['newGradeId']!="undefined") {
		...
		if(!empty($studentsArr)) {
			foreach($studentsArr as $kk=>$vv) { 
				...
				$fieldsArr=array();
				$fieldsArr[] = "grade_id='".$_POST['newGradeId']."'";
				$fieldsArr[] = "school_id='".$rs_new_grade->school_id."'";
				Student::updateStudentByFields($fieldsArr, $vv);
				
				if($_POST['gradeCircularId']!="" && $_POST['gradeCircularId']!="undefined") {
					$student_id = $vv;
					$circularIds = explode(",", $_POST['gradeCircularId']);
					$action_type = "Student";
	...
	?>
        <div class="grade_division" style="width:25%;"><!-- Grade Division -->
            <table width="230" border="0" cellspacing="0" cellpadding="0" style="margin:0 auto; cursor:pointer;" class="border_theme">
              <tr>
                <td colspan="2" class="bgbrown2 txtcenter f20 txtbold padtb10">
        ...
        <div class="fullsize padtb10"><!-- Grade Tab-->
            ...
            <div class="gradetab_outer" style="width:98%; margin:0 1%;">
                ...
                <div class="pull_right">
                    ...
                    <? if($GLOBALS['isAdd']){ ?><div class="gradeAddition pull_left bgbrown2 padlr10 padtb5 txtwhite txtbold f18 cursor margintop5 marginright5 grade_S" onclick="showCirculars('<?=$gradeId?>', 'GS')">SEND CIRCULAR</div><? } ?>
                    <? if($_SESSION['viewLog']) { ?><div class="gradeAddition pull_left bgbrown2 padlr10 padtb5 txtwhite txtbold f18 cursor margintop5 grade_S" onclick="showLogDetails('<?=TBL_GRADE_STUDENTS?>', '')">LOGS</div><? } ?>
                    
                    <? if($GLOBALS['isAdd']){ ?><div class="gradeAddition pull_left bgbrown2 padlr10 padtb5 txtwhite txtbold f18 cursor margintop5 marginright5 grade_T" id="" style="display:none;" onclick="showTeacherFrm('N', '<?=$gradeId?>', '')">ASSIGN TEACHER</div><? } ?>
                    <? if($_SESSION['viewLog']) { ?><div class="gradeAddition pull_left bgbrown2 padlr10 padtb5 txtwhite txtbold f18 cursor margintop5 grade_T" onclick="showLogDetails('<?=TBL_GRADE_TEACHERS?>', '')">LOGS</div><? } ?>
                    
                    <? if($GLOBALS['isAdd']){ ?><div class="gradeAddition pull_left bgbrown2 padlr10 padtb5 txtwhite txtbold f18 cursor margintop5 marginright5 grade_SB" id="" style="display:none;" onclick="showSubFrm('N', '<?=$gradeId?>', '')">ASSIGN SUBJECT</div><? } ?>
                    <? if($_SESSION['viewLog']) { ?><div class="gradeAddition pull_left bgbrown2 padlr10 padtb5 txtwhite txtbold f18 cursor margintop5 grade_SB" onclick="showLogDetails('<?=TBL_GRADE_SUBJECTS?>', '')">LOGS</div><? } ?>
    ...
    <table width="450" border="0" cellspacing="0" cellpadding="0" class="popuptbl">
        ...
        <tr>
        	<td colspan="2" align="right">
            	<div class="fullsize txtwhite txtcenter f18">
                <? if(count($rs_grade_subjs)>0){ ?>
                    <div class="bgbrown pull_right marginleft20 margintb10 cursor padlr20 padtb10" onClick="showSubFrm('S', '<?=$_POST['gradeId']?>','<?=$grade_subject_id?>')" id="subjectSave"><strong>Save</strong></div>
                    <div class="bgbrown pull_right marginleft20 margintb10 cursor padlr20 padtb10" id="subjectLoading" style="display:none;"><strong>Processing..</strong></div>
                <? } ?>
                </div>
            </td>
        </tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="popuptbl">
        ...
        <tr>
        	<td colspan="3" align="right">
            	<div class="fullsize txtwhite txtcenter f18">
                    <div class="bgbrown pull_right marginleft20 margintb10 cursor padlr20 padtb10" id="assingStudentSave" onClick="submitNewStudents('<?=$_POST['grade_id']?>')"><strong>Save</strong></div>
                    <div class="bgbrown pull_right marginleft20 margintb10 cursor padlr20 padtb10" id="assingStudentLoading" style="display:none;"><strong>Processing..</strong></div>
                </div>
            </td>
        </tr>
    </table>
        ...
        <div class="pull_left">
            <table width="260" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td>List <br/> <span class="f30"><strong>Campus</strong></span></td>
              </tr>
            </table>
        </div>
        <div class="pull_right f24 padtop50 cursor" id="show_addteacher">
            <? if($_SESSION['viewLog']) { ?><div class="combutton" onclick="showLogDetails('<?=TBL_GRADE?>', '')">Logs</div><? } ?>
        </div>
    </div>
...
<div class="fullsize">
        ...
        <div class="fullsize padtb15">
            ...
            <div class="newsletter_left"> <!-- Grade Menu -->
            	<div class="newsletter_submenu txtwhite">
					<div class="circular_outer">
                    	<div class="newcircular_head" id="show_currentteacher">Campus</div>
            ...
            <div class="newsletter_right border_theme bgwhite" id="gradelisttab" style="width:78.8%;"></div><!-- Grade -->
...
function showSubFrm(action, grade_id, sub_id) { 
	...
	if(action=="N") {
		...
		if(err==0) {
			$('#subjectSave').hide(); $('#subjectLoading').show();
			ajax({
				a:'grade',
...
function submitNewStudents(grade_id) {
	...
	$('#assingStudentSave').hide();
	$('#assingStudentLoading').show();
	ajax({
		a:'grade',
