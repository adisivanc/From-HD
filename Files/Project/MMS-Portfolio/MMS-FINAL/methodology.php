...
<div id="workflow_container">
	...
	<div id="workflow_title">
        ...
        <input type="text" id="current_cir" value="1" />        
    </div>
...
function flowprevnext(btn){
	...
	var selectedCir = parseInt($('#current_cir').val());

	if(btn=='prev'){
		
		if(selectedCir==1){
			
			$('#workflow_title_left').hide();
			$('#workflow_title_right').show();	
			
			$('#workflow_arrow_left').text('# 0');
			$('#workflow_arrow_right').text('# 2');	
			
			$('#flowhd_left').text(myArray[selectedCir]);
			$('#flowhd_right').text(myArray[selectedCir]);	
			
		}else if(selectedCir > 1 && selectedCir < 16){
			$('#workflow_title_left').show(selectedCir-1);
			$('#workflow_title_right').show(selectedCir+1);
			
			$('#workflow_arrow_left').text('# 0');
			$('#workflow_arrow_right').text('# 2');	
			
			$('#flowhd_left').text(myArray[selectedCir]);
			$('#flowhd_right').text(myArray[selectedCir+2]);	
			
		}else{ 
			$('#workflow_title_left').show();
			$('#workflow_title_right').hide();
		...
		newCir = parseInt($('#current_cir').val())+1;

		if(newCir==1){
			$('#workflow_title_left').show();
			$('#workflow_title_right').show();
			
			$('#workflow_arrow_left').text('# '+(selectedCir-1));
			$('#workflow_arrow_right').text('# '+(selectedCir+1));	
			
			$('#flowhd_left').text(myArray[selectedCir-1]);
			$('#flowhd_right').text(myArray[selectedCir]);	
			
		}else if(newCir > 1 && newCir < 16){
			$('#workflow_title_left').show();
			$('#workflow_title_right').show();	
			
			$('#workflow_arrow_left').text('# '+(newCir-1));
			$('#workflow_arrow_right').text('# '+(newCir+1));	
			
			$('#flowhd_left').text(myArray[newCir-2]);
			$('#flowhd_right').text(myArray[newCir]);	
	...
	if(newCir>=7 && newCir<=16)
	{
		for(j=1; j<=16; j++){
			if(j>=(newCir-6) && j<=newCir)
			$('#cir'+j).show('slow');
			else
			$('#cir'+j).hide('slow');	
		}
