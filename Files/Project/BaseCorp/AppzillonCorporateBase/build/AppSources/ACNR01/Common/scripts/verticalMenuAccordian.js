    ...
    accordion: function(options) {
 	...
 	debugger;	
 		...
 		$this.find("li.active").each(function() {
 		     $(this).find('ul').not("ul:first").slideUp(opts.speed, function(event){
  		...
  		$this.find("li a").click(function(event) {
  			...
  			if($(this).parent().find("ul").size() != 0){
  			    $(this).closest('ul').find('ul').slideUp(opts.speed, function(event){
  					$(this).parent("li").find("i:first").delay(opts.speed).html(opts.closedSign);
                });
  				if(opts.accordion){
  						...
  						visible.each(function(visibleIndex){
  							...
  							if(close){
  								if($(this).parent().find("ul") != visible[visibleIndex]){
  									$(visible[visibleIndex]).slideUp(opts.speed, function(event){
  				...
  				if($(this).parent().find("ul:first").is(":visible")){
  					$(this).parent().find("ul:first").slideUp(opts.speed, function(event){
  						$(this).parent("li").find("i:first").delay(opts.speed).html(opts.closedSign);
  					});
  					...
  					$(this).parent().find("ul:first").slideDown(opts.speed, function(event){
