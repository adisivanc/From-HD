...
function constructKeyboard(bool) {
  //  var check;
    ...
    var str_trco = "</tr><tr>";
    sHTML =
        "<table	border='0' class='keyboardtb_new' cellspacing='3px' id='ACLI01__Login__keypad' width='95%'><tr><td class='key_head' colspan='13'>Virtual Keypad</td></tr><tr>";
    var dHTML = "";
    for (var i = 0; i < keyArr.length - 1; i++) {
        ...
        for (var j = 0; j < keyArr[i].length; j++) {
            ...
            if (typeof(keyArr[i][j]) == 'object') {
                while (keyArr[i][j].length > 0) {
                    ...
                    sHTML += "<td onClick='putChar(" + code + ");constructKeyboard(1)' class='keyboardtblenb'>" + ch + "</td>";
                }
            } else {
                code = keyArr[i][j].charCodeAt(0);
                 sHTML += "<td onClick='putChar(" + code + ");constructKeyboard(1)' class='keyboardtblenb'>" + keyArr[i][j] + "</td>";
        ...
        sHTML +=
            "<td colspan='5' id='cap' onClick='setCaps(this)' class='keyboardtblenb' style='background:#E9EFF3;padding:5px 7px; width:100px; font-size:85%;'>CAPS LOCK</td>";
        sHTML +=
            "<td colspan='5' id='clr' onClick='setClearAll()' class='keyboardtblenb' style='background:#E9EFF3;padding:5px 7px; width:100px; font-size:85%;'>CLEAR</td>";
    ...
    for (var i = 0; i < 3; i++) {
        var code = codeArray[i].charCodeAt(0);
         sHTML += "<td onClick='putChar(" + code + ");constructKeyboard(1)' class='keyboardtblenb' >" + codeArray[i] + "</td>";
        
    }
    sHTML += "</tr></table>";
    document.getElementById('ACLI01__Login__keypad').innerHTML = sHTML;
}
shuffle = function(v) {
...
function changeCase() {
    var alphabets = document.getElementById('ACLI01__Login__keypad').getElementsByTagName('TD');
...
function backspacevk() {
    ...
    //alert(profpass.value);var pfvaluevk = profpass.value;
