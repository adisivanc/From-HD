...
apz.bilpay.AddBiller.fnInitialise = function(params) {
    ...
    if (params.customerContext.Navigation) {
        ...
        lParams.showImage = "N";
        lParams.showHome = "Y";
        lParams.headerText = apz.lits['bilpay'][apz.language]['LIT_ADDBILLER'];
...
apz.bilpay.AddBiller.fnPopulateAcc = function() {
    ...
    debugger;
    var lRecs = [{
        "accountNo": "5785936513734",
        "accountType": "Checking",
        "accountBalance": "10,00,996"
    }, {
        "accountNo": "7356329872139",
        "accountType": "Checking",
        "accountBalance": "1,88,373"
    }, {
        "accountNo": "8274239482711",
        "accountType": "Checking",
        "accountBalance": "2,84,748"
    }];
    for (i = 0; i < lRecs.length; i++) {
        
        var strlen = lRecs[i].accountNo;
        strlen = strlen.substr(0, strlen.length - 4).replace(/[0-9]/g, 'X') + strlen.substr(strlen.length - 4, strlen.length).replace(/[0-9]/g, '9');
        var laccNo = lRecs[i].accountNo;
        var result = apz.getMaskedValue(strlen, laccNo);
        
        lArr.push({
            "val": result,
            "desc": lRecs[i].accountType + "- " + result
        });
    }
...
apz.bilpay.AddBiller.fn_confirm = function() {
    ...
    req.billerDtls.customerId = "CorpUser";
    ...
    apz.bilpay.AddBiller.fn_OTPCallback({});
};
...
apz.bilpay.AddBiller.fn_OTPCallback = function(presp) {
    ...
    lParams.headerText = apz.lits['bilpay']['LIT_Success'];
