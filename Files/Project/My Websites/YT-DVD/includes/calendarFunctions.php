...
function getSchoolCalMonthCol($argvs=array()){
	...
	$cal_Icon=''; $alt="";$title="";
	$rs_calendar=Calendar::getCalendarByDate($school_id,$eventDate);
	if($_SESSION['YTUserType']=="SA" ){
		$rs_calendar=Calendar::getCalendarByDate($school_id,$eventDate);
		
		$cal_Icon='images/plus_icon.png'; $alt="Add Event";$title="Add Event";
		$date_background="";
	...
	if($_SESSION['YTUserType']=="T"){
					...
					if($dayName!="Sunday")
						{
					$cal_Icon='images/leave_icon.png'; $alt="Apply leave";$title="Apply leave";
						}
					$teacher_obj=new Teacher();
					$teacher_obj->user_name=$_SESSION['YTUserName'];
					$teacher_obj->fields="";			
...
<div  style="text-align:left; padding-top:30px;">
	...
	<?
		$execution=0;
	 	if(count($rs_calendar)>0)
        foreach($rs_calendar as $K=>$V) {
		$chkdate=getDaysBetweenDates($V->event_date,$eventDate);	
        if($chkdate==0) {
		$execution++;
		
		?>
             <div onclick="<? if($_SESSION['YTUserType']=="SA"){ ?>addCalendarEvent_load('<?=$V->id?>','<?=date('Y-m-d',strtotime($V->event_date))?>')<? }elseif($_SESSION['YTUserType']=='PARENT'){?>showDetailsToParents('<?=$V->id?>','<?=date('Y-m-d',strtotime($V->event_date))?>')<? } elseif($_SESSION['YTUserType']=="T"){
				 ?> showDetailsToTeachers('<?=$V->id?>','<?=date('Y-m-d',strtotime($V->event_date))?>')<? } else {?> showDetailsToParents('<?=$V->id?>','<?=date('Y-m-d',strtotime($V->event_date))?>')   <? } ?>" id="<?=$V->id?>" style="cursor:pointer; background-color:#DAB791; font-size:12px; margin:2px 3px 0 3px; border-radius:3px; padding:1px 0; text-indent:3px;" align="center"><? echo substr($V->event_name,0,10); if(strlen($V->event_name)>10){echo "...";}else{}?></div>
    <? 	}   }
		//show leave status
		if($leave_status!='') {
		?>
